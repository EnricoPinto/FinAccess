...
const SAMPLE_LITERACY_MODULES = [
  {
    ...
    readTimeMinutes: 3,
    content: `An emergency fund is a dedicated pool of liquid cash set aside exclusively for unexpected financial shocks — such as medical emergencies, sudden job transition, or urgent vehicle/home repairs.
...
*Example*: If your essential monthly expenses equal ₹20,000, your target Emergency Fund is ₹60,000 to ₹1,20,000.`,
  ...
  },
  {
    ...
    readTimeMinutes: 3,
    content: `A classic dilemma for young earners: If I have ₹50,000 in credit card debt, should I save for an emergency fund or pay down debt?
...
3. **Step 3 — Expand to Full 6-Month Fund**: Once high-cost debt is gone, route those freed-up monthly EMI payments into your primary emergency reserve.`,
  ...
  },
  {
    ...
    readTimeMinutes: 4,
    content: `Your Debt-to-Income (DTI) ratio measures how much of your gross monthly income goes toward existing debt payments (credit card EMIs, education loans, bike loans).
...
- **> 50%**: High risk. Most formal banks will decline unsecured loan applications until existing debts are cleared.`,
  ...
  },
  {
    ...
    readTimeMinutes: 4,
    content: `In India, bureaus like CIBIL, Experian, and CRIF High Mark generate a three-digit score between 300 and 900 summarizing your credit repayment behavior.
...
*Pro-Tip*: A score above 750 typically unlocks 0.5% to 1.5% lower interest rates on home and personal loans.`,
  ...
  },
  {
    ...
    readTimeMinutes: 3,
    content: `When comparing loans, many lenders advertise an attractive nominal interest rate like Personal Loans at 9.99%!. However, the Annual Percentage Rate (APR) reveals the true cost.
...
*Rule of Thumb*: Always ask the bank for the Key Fact Statement (KFS) mandated by the Reserve Bank of India (RBI), which explicitly prints the APR.`,
  ...
  },
  {
    ...
    readTimeMinutes: 4,
    content: `Credit cards are powerful tools when used as an interest-free charge card, but dangerous when treated as borrowed wealth.
...
- Treat the credit card like a debit card: never swipe for what you don't already have in your savings account.`,
  ...
  },
  {
    ...
    readTimeMinutes: 3,
    content: `The 50/30/20 framework splits your net take-home pay into three intuitive buckets:
...
- Additional debt principal prepayments`,
  ...
  },
  {
    ...
    readTimeMinutes: 4,
    content: `Albert Einstein famously called compound interest the Eighth Wonder of the World. In personal finance, compounding works best when given time, not just large capital.
...
- Takes advantage of Rupee Cost Averaging: you automatically buy more mutual fund units when markets drop, and fewer when markets rise.`,
  ...
  },
  {
    ...
    readTimeMinutes: 3,
    content: `Most people think money left in a bank account is completely safe. While the nominal rupee balance won't drop, its purchasing power shrinks every single year due to Inflation.
...
3. **Tier 3 (Wealth Growth)**: Long-term savings (5+ years) invested in low-cost Nifty 50 index funds or diversified equity mutual funds to generate inflation-beating 11-13% returns.`,
  ...
  },
  {
    ...
    readTimeMinutes: 4,
    content: `A single medical emergency can wipe out three years of disciplined savings in 48 hours. Insurance is not an investment to make profit; it is defensive armor to protect your hard-earned wealth.
...
- Buy a personal standalone health policy (₹5-10 Lakh base) with a Super Top-up policy (₹25-50 Lakhs) for very low annual premiums in your 20s.`,
  ...
  },
  {
    ...
    readTimeMinutes: 4,
    content: `In India, salaried earners can choose between two tax systems each financial year. Understanding which one fits your income saves thousands in deductions.
...
async function seed() {
  console.log("Resetting transactions & goals for clean user profile...");
  ...
  // Always sync latest real-world loan products
  await prisma.loanProduct.deleteMany({});
  for (const lp of SAMPLE_LOAN_PRODUCTS) {
    await prisma.loanProduct.create({ data: lp });
  }
  console.log(`Seeded ${SAMPLE_LOAN_PRODUCTS.length} real-world loan products.`);

  // Upsert all bite-sized literacy modules
  for (const lm of SAMPLE_LITERACY_MODULES) {
    await prisma.literacyModule.upsert({
      where: { slug: lm.slug },
      update: lm,
      create: lm,
    });
  }
  console.log(`Seeded ${SAMPLE_LITERACY_MODULES.length} bite-sized literacy modules.`);

  console.log("Clean user slate ready with real-world loans and literacy modules!");
}
