...
async function syncAll() {
  console.log("Syncing Real Loans & Literacy Cards to SQLite...");

  // Clear existing loan products and re-populate with actual bank products
  await prisma.loanProduct.deleteMany({});
  for (const lp of SAMPLE_LOAN_PRODUCTS) {
    await prisma.loanProduct.create({ data: lp });
  }
  console.log(`Successfully synced ${SAMPLE_LOAN_PRODUCTS.length} real-world loan products.`);

  // Upsert all literacy modules
  ...
  console.log(`Successfully synced ${SAMPLE_LITERACY_MODULES.length} bite-sized literacy modules.`);
}
